...
export default class CreatePostController extends Component<P, S> {
  ...
  handlePrice = (val: string) => this.setState({price: val});
  ...
  validateAndCreatePost = async () => {
    const {imagePath, price, title, description} = this.state;
    ...
    if (title.length <= 5 || price.length < 0 || description.length <= 10) {
      Alert.alert(
        'Validation Error',
    ...
    if (authId) {
      await createPost(authId, imagePath, price, title, description);
      this.setState({
        imagePath: '',
        price: '',
        title: '',
